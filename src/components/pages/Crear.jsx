...
const Crear = () => {
  ...
  const guardarArticulo = async (e) => {
    ...
    setResultado(datos?.status === "success" ? "guardado" : "error");
  };
    ...
    <div className="jumbo">
      ...
      <form className='formulario' onSubmit={guardarArticulo}>
        ...
        <div className='form-group'>
          ...
          <textarea name='contenido' onChange={cambiado} onInput={ajustarAltura} placeholder='Escribe tu artículo...' style={{ overflow: "hidden", resize: "none" }}/>
        </div>

        {camposContenido.map((contenido, index) => (
          <div className='form-group' key={index}>
            <label htmlFor={`contenido${index + 1}`}>Contenido {index + 1}</label>
            <textarea value={contenido} onChange={(e) => manejarContenido(index, e.target.value)} onInput={ajustarAltura} placeholder={`Párrafo ${index + 1}`} style={{ overflow: "hidden", resize: "none" }} />
          </div>
        ))}
